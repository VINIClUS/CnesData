...
`AGENT_DELTA_MODE` defaults **enabled** (Phase C cutover, 2026-05-03). Set `AGENT_DELTA_MODE=false` to opt out (legacy full-snapshot escape hatch). Enabled = SHA-256 row-fingerprint delta for cnes/sihd/bpa. State store at `%PROGRAMDATA%\dumpagent\state\delta.db` (bbolt). Per (source, intent, competencia): `committed/<path>` sub-bucket holds last-cycle hashes; `pending/<path>/<job_id>` holds in-flight; atomic tx-swap on CompleteJob ack via P5.2 outbox. Delta Parquet emits `_op ∈ {I,U,D}` column; D rows carry PK-only. SIA stays full-extract. 24h GC of stale pending on Open. `internal/delta/` package complete (types/profiles/fingerprint/compute/store/writer); executor helper `ComputeAndStagePending` ready. data_processor must run `DELTA_MODE=true` to accept `_op` parquet via `cdc_merger.merge_delta`. Cold-start emits all-I + WARN log. New diagnose check `delta_store`. Default `AGENT_DELTA_MODE` unset → legacy full-snapshot path; flip per tenant for pilot.
