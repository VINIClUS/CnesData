...
const _ITEMS: { label: string; href: string; icon: ComponentType<IconProps> }[] = [
  { label: "LinkedIn", href: marketing.social.linkedin, icon: LinkedInIcon },
  { label: "GitHub", href: marketing.social.github, icon: GitHubIcon },
  { label: "E-mail", href: `mailto:${marketing.contactEmail}`, icon: Mail },
];

export function SocialLinks({ className }: { className?: string }) {
  return (
    <div className={cn("flex items-center gap-4", className)}>
      {_ITEMS.map(({ label, href, icon: Icon }) => (
        <a
          key={label}
          href={href}
          aria-label={label}
