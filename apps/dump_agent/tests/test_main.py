...
@patch("dump_agent.main.PipelineOrchestrator")
@patch("dump_agent.main.parse_args")
@patch("dump_agent.main.configurar_logging")
...
def test_main_usa_nova_ordem_de_stages():
    import ast
    src = Path("apps/dump_agent/src/dump_agent/main.py").read_text(encoding="utf-8")
    tree = ast.parse(src)
    from_imports = {
        alias.name
