...
def _state(target_source: str = "AMBOS") -> PipelineState:
...
@patch("dump_agent.stages.ingestao_nacional.CnesNacionalAdapter")
@patch("dump_agent.stages.ingestao_nacional.config")
def test_popula_state_quando_target_source_ambos(mock_config, mock_adapter_cls):
...
def test_busca_profissionais_e_estabelecimentos_em_paralelo(mock_config, mock_adapter_cls):
    ...
    mock_adapter.listar_estabelecimentos.side_effect = (
        lambda c: calls.append("estab") or _df_estab()
    )
    mock_adapter_cls.return_value = mock_adapter

    IngestaoNacionalStage().execute(_state("AMBOS"))
...
@patch("dump_agent.stages.ingestao_nacional.CnesNacionalAdapter")
@patch("dump_agent.stages.ingestao_nacional.config")
def test_popula_dfs_nacionais_apos_busca(mock_config, mock_adapter_cls):
    mock_config.GCP_PROJECT_ID = "proj"
