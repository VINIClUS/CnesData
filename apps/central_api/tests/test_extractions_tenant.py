...
def test_enqueue_define_tenant_do_corpo_antes_de_gravar(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(config, "ADMIN_TOKEN", "chave-de-teste")
    app = create_app()
    ...
    try:
        with patch("central_api.routes.extractions.extractions_repo.enqueue", _enqueue):
            resp = TestClient(app).post(
                ...
                headers={"X-Admin-Token": "chave-de-teste", "X-Tenant-Id": "999999"},
            )
    finally:
