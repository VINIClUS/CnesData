...
- Infrastructure may be planned after CND-020…025; application promotion additionally requires AWS-010…014 and the serial AWS gate. CND-020…025 are satisfied (#122-#127, closed 2026-09-01..2026-09-06). AWS-010…014 are entirely unimplemented and are gated behind EPIC #94's controller Task 8, which queues behind CND-064 (#222 / PR #227, in flight).
- Existing workflows on develop: ci.yml, python-quality.yml, web-dashboard.yml, sonar.yml, trivy.yml, shadow-e2e.yml, nightly.yml, deploy-develop.yml, deploy-main.yml, claude.yml, claude-code-review.yml and three dump-agent-go workflows. The required-checks-long-lived ruleset (main/develop) requires lint-test-coverage, sonar, dependencies, config and image (central_api | data_processor | cnes_db_migrator | web_dashboard). New workflows in this plan extend that set; they do not replace it.
- deploy-develop.yml and deploy-main.yml run on self-hosted Proxmox runners holding SSH credentials to the Hostinger VPS. New promotion/plan/apply workflows in this plan must stay unreachable from pull_request and must not add a self-hosted runs-on to any job a PR can trigger.
